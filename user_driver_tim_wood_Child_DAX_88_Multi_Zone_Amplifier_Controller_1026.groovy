/* 8-2-2024
child driver fo DAX-88 8 zone audio 
This driver is to control the DAX-88 Multi Zone Amplifier. 
...
*/
metadata {
    definition (name: "Child DAX-88 Multi Zone Amplifier Controller", namespace: "tim.wood", author: "Tim Wood") {
    capability "Switch"
...
def setLevel (Percent){
    ...
    try {
	    ...
	    parent.sendMsg ("<0${state.zone}VO${strvolume}")
  } catch (Exception e) {
...
def on() {
	...
	try {
        ...
        sendEvent(name: "status", value: "on", isStateChange: true)
        state.status = "on"
		parent.sendMsg ("<0${state.zone}PR01")
        }
	 catch (Exception e) {
...
def off() {
    ...
    try {
		...
		parent.sendMsg ("<0${state.zone}PR00")
        }
	catch (Exception e) {
        log.warn "Call to off failed: ${e.message}"
    }
...
def mute() {
    ...
    try {
	    	state.mute = "muted"
		sendEvent(name: "mute", value: "muted", isStateChange: true)
	    	parent.sendMsg ("<0${state.zone}MU01")
  } catch (Exception e) {
        log.warn "Call to off failed: ${e.message}"
    }
}
def unmute() {
    ...
    try {
	    	state.mute = "unmuted"
		sendEvent(name: "mute", value: "unmuted", isStateChange: true)
	    	parent.sendMsg ("<0${state.zone}MU00")
  } catch (Exception e) {
        log.warn "Call to off failed: ${e.message}"
    }
}
def Source1(){
    ...
    try {
        ...
        state.trackDescription = "Source: ${state.mediaSource}"
        sendEvent(name: "trackDescription", value: state.trackDescription, isStateChange: true)
        parent.sendMsg ("<0${state.zone}CH0${state.mediaSource}")
    } catch (Exception e) {
        log.warn "Call to off failed: ${e.message}"
    }
}
...
def Source7(){
    if (logEnable) log.debug "Source7"
    try {
		state.mediaSource = 7
		sendEvent(name: "mediaSource", value: 7, isStateChange: true)
        state.trackDescription = "Source: ${state.mediaSource}"
        sendEvent(name: "trackDescription", value: state.trackDescription, isStateChange: true)        
		parent.sendMsg ("<0${state.zone}CH0${state.mediaSource}")
  } catch (Exception e) {
...
def Source8(){
    if (logEnable) log.debug "Source8"
    try {
		state.mediaSource = 8
		sendEvent(name: "mediaSource", value: 8, isStateChange: true)
        state.trackDescription = "Source: ${state.mediaSource}"
        sendEvent(name: "trackDescription", value: state.trackDescription, isStateChange: true)        
		parent.sendMsg ("<0${state.zone}CH0${state.mediaSource}")
  } catch (Exception e) {
